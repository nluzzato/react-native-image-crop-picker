...
RCT_EXPORT_METHOD(openPicker:(NSDictionary *)options
    ...
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSString *color = [self.options objectForKey:@"tintColor"];
            if (color) {
                unsigned rgbValue = 0;
                NSScanner *scanner = [NSScanner scannerWithString:color];
                [scanner setScanLocation:1]; // bypass '#' character
                [scanner scanHexInt:&rgbValue];
                UIColor *tintColor = [UIColor colorWithRed:((rgbValue & 0xFF0000) >> 16)/255.0 green:((rgbValue & 0xFF00) >> 8)/255.0 blue:(rgbValue & 0xFF)/255.0 alpha:1.0];
                [[imagePickerController view] setTintColor:tintColor];
            }
